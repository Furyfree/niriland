...
Niriland is in maintenance mode and is being superseded by Nimbus. The Bash
foundation below is retained work, not a restart of the rewrite. Unfinished
phases in [`docs/PLAN.md`](docs/PLAN.md) and [`docs/ROADMAP.md`](docs/ROADMAP.md)
are deferred.
...
- The repository is expected at `~/.local/share/niriland`; existing deployed
  configurations still reference that path.
- The unsafe TPM/FDE step has been removed. Its replacement is deferred.
...
The existing fresh-install entrypoints remain available:
...
It still upgrades packages and replays selected installer steps. The planned
replacement in Phase 4 is deferred.

Compatibility paths preserve existing configs and copied helpers after the
directory moves. They forward to `configs/home`, `configs/shared` and the
current updater; no manual deployment is required just to keep those paths.
