...
Replace the current `configs/base` and `configs/modules` split with one
canonical `configs/` tree. For example,
...
symlinks into a user-owned Git repository.
...
- `update` previews the upstream Git diff, fast-forwards after approval,
  performs a full system update, runs pending migrations once, and then
  plans/applies additive and corrective drift. It reports undeclared packages
...
`niriland-update`, `niriland-pkg`, `niriland-sync-base-config`, and all setup
scripts should not survive as independent implementations. Thin compatibility
wrappers may temporarily delegate to `niriland`, then be removed with documented
migration notes.
...
6. perform a full package upgrade;
...
- Vesktop: the working local portal routing is `default=gnome;gtk`, with GTK for
  Access/Notification and gnome-keyring for Secret. The Vencord
...
- move `configs/base` and `configs/modules` into the canonical structure;
- trace all existing include references before moving files;
...
- correct portal routing using the working Vesktop state as evidence;
...
- implement `apply --prune` with a protected set, snapshot, and transaction
  guard;
- migrate or remove old `niriland-*` wrappers;
