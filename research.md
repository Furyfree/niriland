...
- Keep the working Niri portal routing. Vesktop's Vencord screen-share plugin is
  a quality tweak, not the mechanism that makes portal capture work.
...
## Vesktop screen sharing and portals

The working live portal config on this desktop is:
...
The tracked file currently says only `default=gtk`, so copying the tracked file
would regress the working state.
...
Phase 3 validation must test both a full monitor and an individual window after
the portal change and again after the DMS-to-Noctalia cutover.
...
- `niriland-pkg`: remove after declarative package apply/prune exists. Its
  interactive browse/install function is outside reconciliation and can be a
  separate personal tool if still wanted.
...
- Phase 3 must remove global `GDK_BACKEND` and
  `ELECTRON_OZONE_PLATFORM_HINT`, preserve the working portal routing, and
  describe Vencord's screen-share plugin as an optional bitrate fix.
