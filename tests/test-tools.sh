...
for path in .config/niri/config.kdl .config/ghostty/config .config/zsh/env.zsh; do
  [[ -f "$TEST_ROOT/configs/modules/$path" ]] \
    || fail "existing configuration lost shared file: $path"
done
[[ -f "$TEST_ROOT/configs/base/.local/share/minecraft/Main.mrpack" ]] \
  || fail "old gaming helper lost the Minecraft pack"
[[ -d "$TEST_ROOT/scripts/install/steps" ]] \
  || fail "old updater cannot find installer steps"
legacy_root="$(
  # shellcheck source=scripts/install/lib/common
  source "$TEST_ROOT/scripts/install/lib/common"
  printf '%s' "$REPO_ROOT"
)"
[[ "$legacy_root" == "$TEST_ROOT" ]] \
  || fail "legacy library resolved the wrong repository"

handoff_repo="$test_dir/handoff"
mkdir -p "$handoff_repo/scripts/tools" "$handoff_repo/bin"
cp "$TEST_ROOT/scripts/tools/niriland-update" "$handoff_repo/scripts/tools/"
cat > "$handoff_repo/bin/niriland-update" <<'EOF'
#!/usr/bin/env bash
printf '%s\n' "$#" "$@"
exit 17
EOF
handoff_status=0
handoff_output="$(bash "$handoff_repo/scripts/tools/niriland-update" 'two words' --flag)" \
  || handoff_status=$?
[[ "$handoff_output" == $'2\ntwo words\n--flag' && "$handoff_status" == 17 ]] \
  || fail "legacy updater did not preserve arguments and exit status"
pass "legacy paths preserve configurations and updater handoff"
